...
import loadPersonalityStoryData from '../utill/loadPersonalityStoryData';
...
const PersonalityStoryContent = ({ darkMode }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // 인격 스토리 데이터 로드
        const data = await loadPersonalityStoryData(personalityId, storyId);
        setStoryData(data);
      } catch (error) {
  ...
  return (
    <div className={`${darkMode ? 'bg-neutral-800' : 'bg-white'} p-3 md:p-6 rounded-lg shadow-lg`}>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <h1 className="text-xl md:text-2xl font-bold text-center flex-1">{storyData?.title || '인격 스토리'}</h1>
